package org.nikol.roasti.features.uploads
...
class UploadRepositoryImpl : UploadRepository {
    ...
    override suspend fun save(id: String, contentType: String): Unit = withContext(Dispatchers.IO) {
    ...
    override suspend fun findById(id: String): UploadMeta? = withContext(Dispatchers.IO) {
        val uuid = runCatching { Uuid.parse(id) }.getOrNull() ?: return@withContext null
        transaction {
            UploadTable.selectAll().where { UploadTable.id eq uuid }.singleOrNull()
