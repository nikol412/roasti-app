...
class RecipeRepositoryImpl : RecipeRepository {
    ...
    override suspend fun list(
        ...
        difficulty: Difficulty?,
        roastLevel: RoastLevel?,
    ): Pair<List<RecipeRow>, Int> = withContext(Dispatchers.IO) {
        transaction {
            var cond: org.jetbrains.exposed.v1.core.Op<Boolean> = if (requestingUserId != null) {
    ...
    override suspend fun create(authorId: UserId, input: CreateRecipeInput): RecipeRow = withContext(Dispatchers.IO) {
        val id = Uuid.random()
    ...
    override suspend fun delete(id: RecipeId): Unit = withContext(Dispatchers.IO) {
        transaction {
            RecipeTable.deleteWhere { RecipeTable.id eq id.value }
        }
    }

    override suspend fun getSteps(recipeId: RecipeId): List<BrewStep> = withContext(Dispatchers.IO) {
        transaction {
