...
import org.nikol.roasti.feature.auth.data.network.model.request.RefreshRequestDto
...
class AuthTest {
    ...
    private suspend fun ApplicationTestBuilder.register(
        ...
        val response = jsonClient().post("/api/v1/auth/register") {
            ...
            setBody(RegisterRequestDto(email = email, username = username, password = password, name = name))
        }
    ...
    fun `register - duplicate username returns 409`() = withApp {
        ...
        val response = jsonClient().post("/api/v1/auth/register") {
            contentType(ContentType.Application.Json)
            setBody(RegisterRequestDto(email = "other_$email", username = username, password = password))
        }
    ...
    fun `register - invalid username format returns 422`() = withApp {
        ...
        val response = jsonClient().post("/api/v1/auth/register") {
            contentType(ContentType.Application.Json)
            setBody(RegisterRequestDto(email = email, username = "invalid username!", password = password))
        }
        assertEquals(HttpStatusCode.UnprocessableEntity, response.status)
    }
    ...
    fun `refresh - happy path`() = withApp {
        ...
        val response = jsonClient().post("/api/v1/auth/refresh") {
            contentType(ContentType.Application.Json)
    ...
    fun `refresh - new token is usable`() = withApp {
        ...
        val first = jsonClient().post("/api/v1/auth/refresh") {
            contentType(ContentType.Application.Json)
            setBody(RefreshRequestDto(refreshToken = auth.refreshToken))
        }.body<RefreshResponseDto>()

        val second = jsonClient().post("/api/v1/auth/refresh") {
            contentType(ContentType.Application.Json)
    ...
    fun `logout - happy path`() = withApp {
        ...
        val response = jsonClient(auth.accessToken).post("/api/v1/auth/logout") {
            contentType(ContentType.Application.Json)
