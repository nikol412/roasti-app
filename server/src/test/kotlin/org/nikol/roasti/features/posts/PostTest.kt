...
class PostTest {
    ...
    private val defaultPayload = CreatePostRequestDto(title = "My first post", text = "My first post text")
    ...
    fun `create post - unauthenticated returns 401`() = withApp {
        val response = jsonClient().post("/api/v1/posts") {
            contentType(ContentType.Application.Json)
    ...
    // TODO: GET /posts/{id} is public in Kotlin — decide if auth should be required (Go returns 401 for unauthenticated)

    @Test
    fun `get post - not found returns 404`() = withApp {
        val client = newAuthenticatedClient()
        assertEquals(HttpStatusCode.NotFound, client.get("/api/v1/posts/${UUID.randomUUID()}").status)
    }
    ...
    fun `list posts - returns created posts filtered by author`() = withApp {
        ...
        val page = client.get("/api/v1/posts?author_id=$authorId").body<PageResponseDto<PostResponseDto>>()
    ...
    fun `list posts - respects pagination`() = withApp {
        ...
        val page = client.get("/api/v1/posts?author_id=$authorId&limit=2").body<PageResponseDto<PostResponseDto>>()
    ...
    fun `update post - author can update`() = withApp {
        val client = newAuthenticatedClient()
        val created = createPost(client)
        val response = client.put("/api/v1/posts/${created.id}") {
            contentType(ContentType.Application.Json)
    ...
    fun `update post - non-author returns 403`() = withApp {
        ...
        val response = c2.put("/api/v1/posts/${created.id}") {
            contentType(ContentType.Application.Json)
    ...
    fun `update post - not found returns 404`() = withApp {
        val client = newAuthenticatedClient()
        val response = client.put("/api/v1/posts/${UUID.randomUUID()}") {
            contentType(ContentType.Application.Json)
    ...
    fun `update post - unauthenticated returns 401`() = withApp {
        val client = newAuthenticatedClient()
        val created = createPost(client)
        val response = jsonClient().put("/api/v1/posts/${created.id}") {
            contentType(ContentType.Application.Json)
            setBody(UpdatePostRequestDto(title = "X", text = "x"))
        }
        assertEquals(HttpStatusCode.Unauthorized, response.status)
    ...
    fun `delete post - unauthenticated returns 401`() = withApp {
        val client = newAuthenticatedClient()
        val created = createPost(client)
        assertEquals(HttpStatusCode.Unauthorized, jsonClient().delete("/api/v1/posts/${created.id}").status)
    }
    ...
    fun `vote - upvote increases rating`() = withApp {
        ...
        val response = voter.put("/api/v1/posts/${post.id}/vote") {
            contentType(ContentType.Application.Json)
    ...
    fun `vote - downvote decreases rating`() = withApp {
        ...
        val vote = voter.put("/api/v1/posts/${post.id}/vote") {
            contentType(ContentType.Application.Json)
    ...
    fun `vote - non-existent post returns 404`() = withApp {
        val client = newAuthenticatedClient()
        val response = client.put("/api/v1/posts/${UUID.randomUUID()}/vote") {
            contentType(ContentType.Application.Json)
            setBody(VoteRequestDto(type = VoteDirectionDto.UP))
        }
        assertEquals(HttpStatusCode.NotFound, response.status)
    ...
    fun `vote - unauthenticated returns 401`() = withApp {
        val author = newAuthenticatedClient()
        val post = createPost(author)
        val response = jsonClient().put("/api/v1/posts/${post.id}/vote") {
            contentType(ContentType.Application.Json)
            setBody(VoteRequestDto(type = VoteDirectionDto.UP))
        }
        assertEquals(HttpStatusCode.Unauthorized, response.status)
    ...
    fun `comment - create on own post`() = withApp {
        val client = newAuthenticatedClient()
        val post = createPost(client)
        val response = client.post("/api/v1/posts/${post.id}/comments") {
            contentType(ContentType.Application.Json)
    ...
    fun `comment - another user can comment`() = withApp {
        ...
        val response = c2.post("/api/v1/posts/${post.id}/comments") {
            contentType(ContentType.Application.Json)
    ...
    fun `comment - reply has parent id set`() = withApp {
        ...
        val root = client.post("/api/v1/posts/${post.id}/comments") {
            contentType(ContentType.Application.Json)
            setBody(CreateCommentRequestDto(text = "root"))
        }.body<CommentResponseDto>()

        val reply = client.post("/api/v1/posts/${post.id}/comments") {
            contentType(ContentType.Application.Json)
    ...
    // TODO: add test "comment - non-existent post returns 404" once post existence is validated in comment creation

    @Test
    fun `comment - unauthenticated returns 401`() = withApp {
        val client = newAuthenticatedClient()
        val post = createPost(client)
        val response = jsonClient().post("/api/v1/posts/${post.id}/comments") {
            contentType(ContentType.Application.Json)
    ...
    // TODO: add test "list comments - non-existent post returns 404" once post existence is validated in comment listing

    @Test
    fun `list comments - empty page when no comments`() = withApp {
        val client = newAuthenticatedClient()
        val post = createPost(client)
        val page = client.get("/api/v1/posts/${post.id}/comments").body<PageResponseDto<CommentThreadResponseDto>>()
    ...
    fun `list comments - respects pagination`() = withApp {
        ...
        val page = client.get("/api/v1/posts/${post.id}/comments?limit=2").body<PageResponseDto<CommentThreadResponseDto>>()
        assertEquals(2, page.items.size)
        assertEquals(2, page.pagination.lastPage)
    }
}
